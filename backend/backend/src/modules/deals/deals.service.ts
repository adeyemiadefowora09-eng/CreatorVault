...
export async function completeDeal(dealId: string, userId: string) {
  ...
  // Trust score: reward both parties for a clean completion, but only if no
  // dispute was ever raised on this deal.
  const hadDispute = await prisma.dispute.findFirst({ where: { dealId } });
  if (!hadDispute) {
    await Promise.all([
